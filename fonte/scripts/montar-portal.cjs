...
/* GRADE, NAO FILA.
   Em fila, a primeira coluna esticava e empurrava todas as outras para a direita: sobrava
   um vao enorme depois da marca e as informacoes ficavam amontoadas no canto. Pior, cada
   linha escolhia a propria largura, entao "Ticket" de uma nao ficava embaixo do "Ticket"
   da outra. Com colunas definidas, o espaco se reparte e tudo alinha de cima a baixo.
   O dono estranhou isso olhando a lista de proximos agendamentos, em 22/08/2026. */
/* O RECUO DA ESQUERDA E DE PROPOSITO.
   Antes a linha comecava na MESMA margem do titulo "Proximos agendamentos" — tecnicamente
   alinhado, mas na tela parecia que a marca estava jogada contra a borda. Um recuo pequeno
   separa o conteudo do cabecalho e faz a lista ler como lista, nao como continuacao do
   titulo. O dono pediu esse empurraozinho em 22/08/2026. */
.ag{display:grid;align-items:center;gap:16px;padding:13px 16px 13px 24px;border-bottom:1px solid #f1f4f7;
    grid-template-columns:minmax(170px,1.5fr) minmax(112px,1fr) minmax(84px,.8fr)
                          minmax(140px,1.1fr) minmax(112px,auto) 16px;
...
.ag .seta{color:var(--txt3);justify-self:end}
.ag .seta svg{width:16px;height:16px;display:block}
/* em tela estreita nao cabem cinco colunas: a marca ocupa a linha toda e o resto se
   parte em duas. A seta sai — a linha inteira ja e clicavel, e ali cada pixel conta. */
@media (max-width:900px){
  .ag{grid-template-columns:1fr 1fr;gap:10px 14px}
  .ag-nome{grid-column:1 / -1}
  .ag .seta{display:none}
}
...
/* Pedido que nao tem mais saldo. Reaproveita o cinza do .concluida de proposito:
...
/* rodape da janela de vincular: o que esta marcado + o botao de terminar */
.vp-pe{display:flex;align-items:center;gap:14px;justify-content:space-between;
       margin-top:14px;padding-top:14px;border-top:1px solid var(--borda);flex-wrap:wrap}
.vp-pe-t{font-size:13px;font-weight:700;color:var(--txt2)}
...
/* LINHA DO TEMPO DEITADA.
   Em pe ela empilhava e empurrava o resto da aba para baixo — e o que ela conta e uma
   SEQUENCIA: pedido, documento, cancelado. Deitada, a ordem se le de uma vez, do jeito
   que se le uma linha do tempo. O dono pediu em 22/08/2026.
...
/* o traco que liga um ponto ao proximo; o ultimo nao tem para onde ligar */
.hist li:not(:last-child):before{content:"";position:absolute;left:14px;top:8px;right:0;height:2px;
               background:var(--borda)}
.hist li b{color:var(--txt);display:block;font-size:12.5px;margin-bottom:2px}
@media (max-width:620px){
  .hist{display:block;padding:0 0 0 15px;border-left:2px solid var(--borda);overflow:visible}
  .hist li{padding:0 0 11px;min-width:0}
  .hist li:after{left:-20px;top:5px;width:7px;height:7px}
  .hist li:not(:last-child):before{display:none}
}
...
(function(){
      ...
      if(!prox.length){
        ...
        for(var i=0;i<prox.length;i++){
          ...
          h+='<div class="ag" data-ver="'+esc(a.id)+'">'+
             ...
             '<div class="ag-c"><label>Data da entrega</label><div>'+
                esc(p.curta+(p.hora?" · "+p.hora:""))+'</div></div>'+
             '<div class="ag-c">'+uiSelo(a.situacao)+'</div>'+
  ...
  // Como o pedido fechado se chama na tela. O banco guarda "atendido" e
  // "encerrado", que sao coisas diferentes e por isso tem palavras diferentes:
  //   atendido  = o VR olhou e ele nao tem mais saldo -> foi ENTREGUE
  //   encerrado = o VR nem olha mais para ele (velho demais) -> so ENCERRADO
  // Chamar os dois de "entregue" seria afirmar o que nao se sabe.
  function seloPedido(sit){
    var s=String(sit||"aberto");
    if(s==="atendido")  return '<span class="selo atendido">entregue</span>';
    if(s==="encerrado") return '<span class="selo encerrado">encerrado</span>';
    return '<span class="selo aberto">'+esc(s)+'</span>';
  }

  function colunasPedido(comSaldo){
    var c=[
    ...
    // "Para entrega" e "Saldo" so fazem sentido em cima do que ainda falta.
    // No historico as duas seriam zero em toda linha — coluna que so sabe
    // dizer zero nao informa, so ocupa o olho.
    if(comSaldo){
      c.push({ch:"Para entrega",cl:"nowrap",v:function(p){ return '<b>'+esc(String(p.itens_saldo||0))+'</b>'; }});
    }
    c.push({ch:"Total",cl:"nowrap",v:function(p){ return esc(moeda(p.valor)); }});
    if(comSaldo){
      c.push({ch:"Saldo",cl:"nowrap",v:function(p){ return esc(moeda(p.saldo)); }});
    }
    c.push({ch:"Situação",cl:"nowrap",v:function(p){ return seloPedido(p.situacao); }});
    c.push({ch:"",cl:"nowrap",w:"44px",v:function(p){
        return '<button class="olho" data-pedido="'+esc(p.id)+'" title="Ver pedido">'+IC.olho+'</button>'; }});
    return c;
  }

  function pagPedidos(){
    // DUAS PERGUNTAS, UMA VIAGEM SO. O que falta entregar e o que ja foi vem
    // juntos; buscando em sequencia a tela desenharia duas vezes e piscaria.
    Promise.all([SB.rpc("forn_pedidos"), SB.rpc("forn_pedidos_entregues")]).then(function(rr){
      if(pagAtual!=="pedidos") return;
      var d=deuCerto(rr[0],"os pedidos"); if(d===null) return;
      var l=d.pedidos||[];

      // O HISTORICO NAO PODE DERRUBAR A TELA. Se a segunda pergunta falhar —
      // a funcao ainda nao rodada no banco, por exemplo — o fornecedor
      // continua vendo o que tem para entregar, que e o que ele veio fazer.
    ...
    el("wzAvanca").onclick=function(){
      ...
      if(wz.comNota && wz.pedidosLista && wz.pedidosLista.length){
        ...
        for(var s=0;s<wz.chaves.length;s++){
          if(!temVinc(wz.chaves[s])) soltas.push(String(wz.chaves[s].numero||("nota "+(s+1))));
        }
        if(soltas.length){
  ...
  function paraOndeDepoisDaNota(cb){
    ...
    for(var i=0;i<wz.chaves.length;i++){ if(!temVinc(wz.chaves[i])) return cb("pedidos"); }
    ...
    conferirGrupos().then(function(v){
      ...
      var qs=(wz.chaves.length===1 && temVinc(wz.chaves[0]))
        ? ((wz.chaves[0].vinc.length===1?" com o pedido ":" com os pedidos ")+vincTxt(wz.chaves[0])) : "";
    ...
    for(var i=0;i<wz.chaves.length;i++){
      ...
      if(temVinc(n)){
        col='<div class="nf-ped ok">'+
            '<span class="rot">'+(n.vinc.length>1?"Pedidos":(n.vincAuto?"Pedido":"Pedido manual"))+'</span>'+
            '<b>'+esc(vincTxt(n))+'</b>'+
  ...
  // O VINCULO DA NOTA E UMA LISTA, NAO UM TEXTO.
  //
  // O dono explicou em 22/08/2026: "as vezes tira metade do pedido e a outra metade vai
  // em outro pedido, so que foi emitido um XML so — eu teria que selecionar dois pedidos".
  // Medido nas 3.516 notas reais da loja: 3 em cada 10 tem produto de mais de um pedido.
  // Com um vinculo so, a outra metade caia em "este item nao esta no pedido" — acusacao
  // que BARRA o caminhao, por uma escolha que foi do comprador da loja, nao dele.
  //
  // n.vinc passa a ser um array de numeros de pedido. Vazio = sem vinculo.
  function temVinc(n){ return !!(n && n.vinc && n.vinc.length); }
  function vincLista(n){ return (n && n.vinc) ? n.vinc.slice() : []; }
  // "23102" · "23102 e 23209" · "23102, 23209 e 23301" — do jeito que se fala
  function vincTxt(n){
    var l=vincLista(n);
    if(!l.length) return "";
  ...
  function pedidosDoAgendamento(){
    ...
    for(i=0;i<wz.chaves.length;i++){
      var vs=vincLista(wz.chaves[i]);
      for(var j=0;j<vs.length;j++){ v=String(vs[j]); if(v && l.indexOf(v)<0) l.push(v); }
    }
  ...
  function abrirEscolhaPedido(idx){
    ...
    var c=uiModal({titulo:"Meus pedidos", cru:true,
      corpo:'<div class="mcorpo">'+
        ...
        '<div id="vpLista" class="vp-l"></div>'+
        '<div id="vpPe" class="vp-pe"></div></div>'});

    // O RODAPE DA JANELA. Mostra o que ja esta marcado e fecha quando ele terminar.
    // Sem ele, marcar varios nao teria fim visivel: a pessoa nao saberia quando parar.
    function pintarPeVinc(){
      var pe=el("vpPe"); if(!pe) return;
      var l=vincLista(n);
      pe.innerHTML =
        '<div class="vp-pe-t">'+(l.length
          ? (l.length===1?"Marcado: pedido ":"Marcados: pedidos ")+esc(vincTxt(n))
          : "Nenhum pedido marcado ainda")+'</div>'+
        '<button class="bt'+(l.length?"":" fraco")+'" id="vpPronto">'+
        (l.length?"Pronto":"Fechar")+'</button>';
      el("vpPronto").onclick=function(){
        uiFecharModal();
        sincronizarPedidoTexto();
        listarNotas();
        pintarResumoLado();
        ...
        conferirNota();
        if(vincLista(n).length){
          uiToast("Nota "+(n.numero||"")+" vinculada "+
                  (vincLista(n).length===1?"ao pedido ":"aos pedidos ")+vincTxt(n)+".");
        }
      };
    }
    ...
    function pinta(){
      ...
      for(var i=0;i<vis.length;i++){
        var x=vis[i], on=vincLista(n).indexOf(String(x.numero))>=0;
      ...
      // MARCAR, NAO ESCOLHER. Clicar LIGA e DESLIGA o pedido, e a janela fica aberta:
      // uma nota pode cobrir mais de um pedido, e fechar no primeiro clique obrigaria a
      // reabrir para marcar o segundo — parecendo que so cabe um.
      [].slice.call(el("vpLista").querySelectorAll(".vp-i[data-num]")).forEach(function(e){
        e.onclick=function(){
          var num=e.getAttribute("data-num"), l=vincLista(n), k=l.indexOf(num);
          if(k>=0) l.splice(k,1); else l.push(num);
          n.vinc=l;
          ...
          sincronizarPedidoTexto();
          pintarResumoLado();
          pinta();          // redesenha a lista com o novo estado
          pintarPeVinc();   // e o rodape com o "Pronto"
        };
      });
    }
  ...
  function gruposParaConferir(){
    var mapa={}, ordem=[];
    for(var i=0;i<wz.chaves.length;i++){
      var n=wz.chaves[i], vs=vincLista(n), its=n.itens||[];
      if(!vs.length || !its.length) continue;
      // a chave do grupo e o CONJUNTO de pedidos: notas que apontam exatamente os
      // mesmos pedidos conferem juntas, senao "nao vieram nesta nota" conta em dobro
      var v=vs.slice().sort().join("|");
      if(!mapa[v]){ mapa[v]={pedidos:vs, itens:[]}; ordem.push(v); }
      for(var j=0;j<its.length;j++){
        var t=its[j];
  ...
  function conferirGrupos(){
    ...
    return Promise.all(gs.map(function(g){
      return SB.rpc("forn_conferir_nota",{p_pedidos:g.pedidos, p_itens:g.itens});
    })).then(function(rs){
  ...
  function avisoNotaSolta(){
    ...
    var soltas=[];
    for(var i=0;i<wz.chaves.length;i++){
      if(!temVinc(wz.chaves[i])) soltas.push({i:i, num:String(wz.chaves[i].numero||("nota "+(i+1)))});
    }
        ...
        setTimeout(function(){
          ...
          if(c){
            ...
            for(var q=0;q<wz.chaves.length;q++){
              if(!temVinc(wz.chaves[q]) && String(wz.chaves[q].vincSolto||"")===n){
                wz.chaves[q].vinc=[n];
          ...
          if(!c){
            ...
            for(var q=0;q<wz.chaves.length;q++){
              var vq=vincLista(wz.chaves[q]), kq=vq.indexOf(n);
              if(kq>=0){
                // tiro SO o pedido desmarcado; os outros vinculos da nota ficam
                vq.splice(kq,1);
                wz.chaves[q].vinc=vq;
                ...
                if(!vq.length) wz.chaves[q].vincAuto=false;
              }
            }
          }
  ...
  function notasParaServidor(){
    ...
    return wz.chaves.map(function(n){
        ...
        return { chave:n.chave, xml:n.xml||null, numero:n.numero, serie:n.serie,
                 ...
                 /* A LISTA. 'pedido' continua indo com o primeiro: enquanto o SQL novo
                    nao rodar, o servidor le dele — e assim nenhuma ordem de publicacao
                    quebra nada, nos dois sentidos. */
                 pedidos:vincLista(n),
                 pedido:(vincLista(n)[0]||null),
                 ...
                 /* OS ITENS VAO COM OS DOIS NOMES DE CAMPO — e por isso que a conferencia
                    de preco nunca funcionou. O leitor do XML monta 'valorUnit' e 'itemPedido'
                    (o jeito do JavaScript), e a funcao do banco procura 'valor_unit' e
                    'item_pedido' (o jeito do Postgres). Nomes diferentes = campo vazio: o
                    servidor recebia preco NULO em toda linha, e o ramo que compara preco
                    exige preco > 0 para disparar. Resultado: NENHUMA nota jamais foi acusada
                    de preco acima do pedido — nem na tela do fornecedor, nem no servidor.
                    Descoberto em 28/08/2026, revisando a trava de preco antes de liga-la.
                    Mando os dois nomes de proposito: a tela do portal le os antigos. */
                 itens: n.itens ? n.itens.map(function(it){
                   return Object.assign({}, it, {
                     valor_unit:  (it.valor_unit  != null ? it.valor_unit  : it.valorUnit),
                     item_pedido: (it.item_pedido != null ? it.item_pedido : it.itemPedido)
                   });
                 }) : null,
                 volumes:n.volumes||null, especie:n.especie||null,
